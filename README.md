...
[![Version](https://img.shields.io/badge/version-6.2.1-brightgreen.svg)](webnovel-writer/.codex-plugin/plugin.json)
...
这套系统做的事，就是把上面这些“必须记住、不能写崩”的约束，变成 Codex 会自动执行的步骤：动笔前先查资料，写完后把新发生的事实记下来、做一致性审查，再把最新状态同步进检索索引、章节摘要、长期记忆和 Dashboard。它不只是“会写”，而是边写边攒。
...
```mermaid
flowchart LR
    User[作者 / Codex] --> Skills[8 个 Skill 命令]
...
- 已安装并登录 Codex，命令行里能执行 `codex`
- Python 3.10+
- macOS / Linux / Windows WSL 推荐；Windows PowerShell 可用，但一键安装脚本建议在 Git Bash 或 WSL 里执行
...
- 安装并启用 `webnovel-writer` 插件
- 安装运行所需的 Python 依赖

安装完成后，**重启 Codex 或至少新开一个对话**，然后输入 `$webnovel-init`。

> 插件的安装、启用与日常管理等更多用法，见 Codex 官方文档：[插件](https://developers.openai.com/codex/plugins) · [构建插件](https://developers.openai.com/codex/plugins/build)。
...
python3 -m pip install --user -r \
  https://raw.githubusercontent.com/BinGess/webnovel-writer/main/requirements.txt
```

如果你已经安装过旧版本，可以刷新 marketplace 后重装：

```bash
codex plugin marketplace upgrade webnovel-writer
codex plugin add webnovel-writer@webnovel-writer
```
...
```bash
# 安装指定分支或 tag
WEBNOVEL_REF=v6.2.1 bash install.sh

# 只安装 Codex 插件，不安装 Python 依赖
INSTALL_PYTHON_DEPS=0 bash install.sh
```
...
### 安装后找不到 `$webnovel-init`

先确认插件状态：

```bash
codex plugin list | grep webnovel-writer
```

正常应看到：

```text
webnovel-writer@webnovel-writer  installed, enabled
```

如果已经 installed/enabled 但新命令没有出现：

- 重启 Codex
- 新开一个对话再输入 `$webnovel-init`
- 如果仍然没有，打开 Codex 的 `/plugins`，确认 `webnovel-writer` 没有被手动关闭

### 更新与卸载

```bash
# 更新 marketplace 快照并重装插件
codex plugin marketplace upgrade webnovel-writer
codex plugin add webnovel-writer@webnovel-writer

# 卸载插件
codex plugin remove webnovel-writer@webnovel-writer
```
...
```bash
python3 -X utf8 "<WEBNOVEL_PLUGIN_ROOT>/scripts/webnovel.py" --project-root "<PROJECT_ROOT>" preflight
python3 -X utf8 "<WEBNOVEL_PLUGIN_ROOT>/scripts/webnovel.py" --project-root "<PROJECT_ROOT>" doctor --format text
```
...
[![Star History Chart](https://api.star-history.com/svg?repos=BinGess/webnovel-writer&type=Date)](https://star-history.com/#BinGess/webnovel-writer&Date)

## 致谢

本项目使用 Codex 与 Gemini CLI 配合 Vibe Coding 方式开发。
