...
| 命名 | 开始命名 | `python3 -X utf8 "${SCRIPTS_DIR}/reference_search.py" --skill init --table 命名规则 --query "{命名对象} {题材}" --genre {题材}` |
...
- Codex subagent：拆分并行子任务；Step 1.5 用户选择参考书拆解作灵感来源时启动 deconstruction-agent。
- 普通对话裁决：关键分歧、候选选择、最终确认必须给有限选项并说明影响。
...
进入故事核采集前，必须先用普通对话确认用户是否提供灵感来源。不要默认拆书，也不要把参考作品当作必填项。
...
当用户选择参考作品拆书且提供文本路径或章节摘录时，必须显式启动 `deconstruction-agent` Codex subagent，不得由 init 主流程口头替代拆解结果。

```text
Spawn Codex custom agent `webnovel-deconstruction-agent`; use `${WEBNOVEL_PLUGIN_ROOT}/agents/deconstruction-agent.md` as the role contract if the custom agent is unavailable.
...
```bash
PROJECT_SLUG="$(python3 -X utf8 -c "import re,sys; title=sys.argv[1].strip(); slug=re.sub(r'[\\\\/:*?\"<>|]+','',title); slug=re.sub(r'\\s+','-',slug).strip('-'); print(('proj-' + slug) if (not slug or slug.startswith('.')) else slug)" "{title}")"
...
参数全部来自上面的采集对象（书名/题材/主角/金手指/世界观/反派/创意约束等），逐字段映射为 `webnovel.py init` 的 `--*` 选项；完整字段清单见 `references/init-collection-schema.md`，可用 `python3 "${SCRIPTS_DIR}/webnovel.py" init --help` 核对选项名。
...
```bash
GENRE="$(python3 -X utf8 -c "import json,os; root=os.environ['PROJECT_ROOT']; s=json.load(open(root + '/.webnovel/state.json',encoding='utf-8')); pi=s.get('project_info',{}); print(pi.get('genre') or s.get('project',{}).get('genre',''))")"
