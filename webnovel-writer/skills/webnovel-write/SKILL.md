...
- 禁止并步、跳步、伪造审查
- 必须显式启动对应 Codex subagent；不得用主流程口头代替 subagent 输出
...
```bash
python3 -X utf8 "${SCRIPTS_DIR}/reference_search.py" --skill write --table {表名} --query "{关键词}" --genre {题材}
```
...
```bash
GENRE="$(python3 -X utf8 -c "import json,sys; s=json.load(open('${PROJECT_ROOT}/.webnovel/state.json',encoding='utf-8')); pi=s.get('project_info',{}); print(pi.get('genre') or s.get('project',{}).get('genre',''))")"
...
必须显式启动 `context-agent` Codex subagent，不得由主流程自行整理任务书。

Spawn Codex custom agent `webnovel-context-agent`; use `${WEBNOVEL_PLUGIN_ROOT}/agents/context-agent.md` as the role contract if the custom agent is unavailable.
...
必须显式启动 `reviewer` Codex subagent，不得由主流程伪造审查 JSON。
...
审查只跑一轮，reviewer 只调用一次。`blocking=true` 的问题在不改剧情、不破设定的前提下定点修复后直接进 Step 4，不重新调用 reviewer；确实无法修复的 blocking 问题用普通对话向用户给出有限选项并等待裁决（接受当前版本 / 手动修复 / 放弃）。非 blocking issue 交给 Step 4 处理。`--fast` 只检查 setting/timeline/continuity。
...
必须显式启动 `data-agent` Codex subagent，产出 fulfillment_result / disambiguation_result / extraction_result 三份 JSON，并复用 Step 3 的 review_results。

Spawn Codex custom agent `webnovel-data-agent`; use `${WEBNOVEL_PLUGIN_ROOT}/agents/data-agent.md` as the role contract if the custom agent is unavailable.
...
```bash
python3 -X utf8 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" run-ledger write-resume \
  --chapter {chapter_num} \
  --mode "{mode}" \
  --format json
```
